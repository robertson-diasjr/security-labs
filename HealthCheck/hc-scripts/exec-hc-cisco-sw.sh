...
do
        ...
        if [[ "$LINE" != *Evaluating ]]
        then
                grep "$LINE" $CONFIG | awk '{print $3}' | while read COMMUNITY; do
		...
		STRING="`echo $COMMUNITY | grep -P '(?=.*[[:lower:]])(?=.*[[:upper:]])(?=.*[[:digit:]])(?=.*[[:punct:]]).{14,}'`"
                ...
                if [ -n "$STRING" ]; then
                        ...
                        # Evaluating string protection (acl)
                        STRING_PROTECTION=`grep -w $STRING $CONFIG | grep '[0-9]$'`
                        if [ -n "$STRING_PROTECTION" ]; then
                                echo "Pass: Access control"
                        else
                                echo "Failed: Access control"
                        fi
                fi
...
#sed -n -e '/SECTION_INTERFACE/,/END_SECTION_INTERFACE/p' $FILE_TEMPLATE | grep -v "SECTION_INTERFACE" | grep -v "^$" | sed -n -e '/^interface/,/^!/p' $FILE_TEMPLATE | sed 's/\!//g' | egrep -v '^interface|^$' > $FILE_IFACE_PARAMETERS
